...
public class ConstraintOccurrence {
	...
	public final static String OCCURRENCE_ANNOTATION_ID = "syntax.resource.constraint.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "syntax.resource.constraint.ui.occurences.declaration";
	...
	private syntax.resource.constraint.ui.ConstraintTokenScanner tokenScanner;
	 ...
	 */
	public ConstraintOccurrence(syntax.resource.constraint.IConstraintTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, syntax.resource.constraint.ui.ConstraintTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new syntax.resource.constraint.mopp.ConstraintMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(syntax.resource.constraint.ui.ConstraintBracketSet bracketSet) {
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, syntax.resource.constraint.ui.ConstraintPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addAnnotation(document, syntax.resource.constraint.ui.ConstraintPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, syntax.resource.constraint.ui.ConstraintPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == syntax.resource.constraint.ui.ConstraintPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(syntax.resource.constraint.ui.ConstraintOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(syntax.resource.constraint.ui.ConstraintOccurrence.DECLARATION_ANNOTATION_ID);
	}

 ...
 * contains
 * <code>hydraconstraints.resource.hydraConst.IHydraConstTextDiagnostic</code>s it
 * uses the more precise information of this extended diagnostic type.
 */
public class HydraConstMarkerHelper {
	
	/**
	 * The extension id of the custom marker type that is used by this text resource.
	 */
	public static final String MARKER_TYPE = hydraconstraints.resource.hydraConst.mopp.HydraConstPlugin.PLUGIN_ID + ".problem";
	...
	private static class MarkerCommandQueue {
		...
		private java.util.List<hydraconstraints.resource.hydraConst.IHydraConstCommand<Object>> commands = new java.util.ArrayList<hydraconstraints.resource.hydraConst.IHydraConstCommand<Object>>();
		private final Object jobLock = new Object();
		
		public void addCommand(hydraconstraints.resource.hydraConst.IHydraConstCommand<Object> command) {
			...
			new org.eclipse.core.runtime.jobs.Job("updating markers") {
				...
				protected org.eclipse.core.runtime.IStatus run(org.eclipse.core.runtime.IProgressMonitor monitor) {
					synchronized(jobLock) {
						java.util.List<hydraconstraints.resource.hydraConst.IHydraConstCommand<Object>> commandsToProcess = new java.util.ArrayList<hydraconstraints.resource.hydraConst.IHydraConstCommand<Object>>();
						synchronized(commands) {
							commandsToProcess.addAll(commands);
							commands.clear();
						}
						for (hydraconstraints.resource.hydraConst.IHydraConstCommand<Object> command : commandsToProcess) {
	 ...
	 */
	public static void mark(org.eclipse.emf.ecore.resource.Resource resource, final hydraconstraints.resource.hydraConst.IHydraConstTextDiagnostic diagnostic) {
	...
	private static void createMarkerFromDiagnostic(final org.eclipse.core.resources.IFile file, final hydraconstraints.resource.hydraConst.IHydraConstTextDiagnostic diagnostic) {
		final hydraconstraints.resource.hydraConst.IHydraConstProblem problem = diagnostic.getProblem();
		hydraconstraints.resource.hydraConst.HydraConstEProblemType problemType = problem.getType();
		final String markerID = getMarkerID(problemType);
		COMMAND_QUEUE.addCommand(new hydraconstraints.resource.hydraConst.IHydraConstCommand<Object>() {
			public boolean execute(Object context) {
				try {
					...
					if (problem.getSeverity() == hydraconstraints.resource.hydraConst.HydraConstEProblemSeverity.ERROR) {
					...
					hydraconstraints.resource.hydraConst.IHydraConstTextDiagnostic textDiagnostic = (hydraconstraints.resource.hydraConst.IHydraConstTextDiagnostic) diagnostic;
					...
					if (diagnostic instanceof hydraconstraints.resource.hydraConst.mopp.HydraConstResource.ElementBasedTextDiagnostic) {
						org.eclipse.emf.ecore.EObject element = ((hydraconstraints.resource.hydraConst.mopp.HydraConstResource.ElementBasedTextDiagnostic) diagnostic).getElement();
					...
					java.util.Collection<hydraconstraints.resource.hydraConst.IHydraConstQuickFix> quickFixes = textDiagnostic.getProblem().getQuickFixes();
					...
					if (!sourceIDs.isEmpty()) {
						marker.setAttribute(org.eclipse.core.resources.IMarker.SOURCE_ID, hydraconstraints.resource.hydraConst.util.HydraConstStringUtil.explode(sourceIDs, "|"));
					}
	...
	public static void unmark(org.eclipse.emf.ecore.resource.Resource resource) {
		for (hydraconstraints.resource.hydraConst.HydraConstEProblemType nextType : hydraconstraints.resource.hydraConst.HydraConstEProblemType.values()) {
	 ...
	 */
	public static void unmark(org.eclipse.emf.ecore.resource.Resource resource, hydraconstraints.resource.hydraConst.HydraConstEProblemType problemType) {
		final org.eclipse.core.resources.IFile file = getFile(resource);
		if (file == null) {
			return;
		}
	...
	public static void unmark(org.eclipse.emf.ecore.resource.Resource resource, final org.eclipse.emf.ecore.EObject causingObject) {
		...
		final String markerID = getMarkerID(hydraconstraints.resource.hydraConst.HydraConstEProblemType.UNKNOWN);
	 ...
	 * given type.
	 */
	private static String getMarkerID(hydraconstraints.resource.hydraConst.HydraConstEProblemType problemType) {
	...
	private static void handleException(org.eclipse.core.runtime.CoreException ce) {
		if (ce.getMessage().matches("Marker.*not found.")) {
			...
			hydraconstraints.resource.hydraConst.mopp.HydraConstPlugin.logError("Error while removing markers from resource:", ce);
		}
	}
}

...
public abstract class AbstractHydraConstDebuggable {
	
	/**
	 * The list of breakpoints, where each breakpoint is represented by its location
	 * (a string) and the line number
	 */
	private java.util.List<hydraconstraints.resource.hydraConst.util.HydraConstPair<String, Integer>> lineBreakpoints = new java.util.ArrayList<hydraconstraints.resource.hydraConst.util.HydraConstPair<String, Integer>>();
	...
	public void startEventSocket(int eventPort) {
		try {
			...
			hydraconstraints.resource.hydraConst.mopp.HydraConstPlugin.logError("Can't create socket connection while launching.", e);
		}
	}
	...
	public void stopEventSocket() {
		try {
			...
			hydraconstraints.resource.hydraConst.mopp.HydraConstPlugin.logError("Exception while closing socket.", e);
		}
	}
	
	public void sendEvent(hydraconstraints.resource.hydraConst.debug.EHydraConstDebugMessageTypes command, boolean sendOnlyInDebugMode, String... arguments) {
		if (isDebugMode() || !sendOnlyInDebugMode) {
			hydraconstraints.resource.hydraConst.debug.HydraConstDebugMessage message = new hydraconstraints.resource.hydraConst.debug.HydraConstDebugMessage(command, arguments);
	...
	public void evaluateLineBreakpoint(org.eclipse.emf.common.util.URI uri, int currentLine) {
		if (isDebugMode()) {
			...
			for (int i = 0; i < lineBreakpoints.size(); i++) {
				hydraconstraints.resource.hydraConst.util.HydraConstPair<String, Integer> breakpointLocationAndLine = lineBreakpoints.get(i);
				...
				if (breakpointLine.intValue() == currentLine && breakpointLocation.equals(location)) {
					// suspending...
					setSuspend(true);
					sendEvent(hydraconstraints.resource.hydraConst.debug.EHydraConstDebugMessageTypes.SUSPENDED, true, new String[] {"breakpoint", "" + currentLine});
					break;
				}
			}
			waitIfSuspended();
	...
	public void waitIfSuspended() {
		...
		sendEvent(hydraconstraints.resource.hydraConst.debug.EHydraConstDebugMessageTypes.RESUMED, true);
	}
	
	public void addLineBreakpoint(String location, int line) {
		lineBreakpoints.add(new hydraconstraints.resource.hydraConst.util.HydraConstPair<String, Integer>(location, new Integer(line)));
	}
	
	public void removeLineBreakpoint(String location, int line) {
		lineBreakpoints.remove(new hydraconstraints.resource.hydraConst.util.HydraConstPair<String, Integer>(location, new Integer(line)));
	}
	...
	public void terminate() {
		sendEvent(hydraconstraints.resource.hydraConst.debug.EHydraConstDebugMessageTypes.TERMINATED, false);
		stopEventSocket();
	}
	...
	public void startUpAndWait() {
		...
		if (isDebugMode()) {
			sendEvent(hydraconstraints.resource.hydraConst.debug.EHydraConstDebugMessageTypes.STARTED, true);
			setSuspend(true);

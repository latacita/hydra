 ...
 */
public class HydraConstMinimalModelHelper {
	
	private final static hydraconstraints.resource.hydraConst.util.HydraConstEClassUtil eClassUtil = new hydraconstraints.resource.hydraConst.util.HydraConstEClassUtil();
			...
			if (feature instanceof org.eclipse.emf.ecore.EReference) {
				...
				if (type instanceof org.eclipse.emf.ecore.EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + hydraconstraints.resource.hydraConst.util.HydraConstStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof java.util.List<?>) {
							java.util.List<org.eclipse.emf.ecore.EObject> list = hydraconstraints.resource.hydraConst.util.HydraConstListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + hydraconstraints.resource.hydraConst.util.HydraConstStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof java.util.List<?>) {
						java.util.List<String> list = hydraconstraints.resource.hydraConst.util.HydraConstListUtil.castListUnchecked(value);

 ...
 */
public class HydraConstBracketSet {
	...
	private java.util.ArrayList<hydraconstraints.resource.hydraConst.IHydraConstBracketPair> bracketPairs;
	 ...
	 */
	private class BracketPair implements hydraconstraints.resource.hydraConst.IHydraConstBracketPair {
	 ...
	 */
	private class ClosingListener implements hydraconstraints.resource.hydraConst.ui.IHydraConstBracketHandler, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.ModifyListener, org.eclipse.swt.custom.VerifyKeyListener {
		...
		public void verifyText(org.eclipse.swt.events.VerifyEvent e) {
			...
			if (caret > 0 && caret < textWidget.getCharCount()) {
				hydraconstraints.resource.hydraConst.IHydraConstBracketPair bracketPair = getBracketPair(textWidget.getTextRange(caret - 1, 1), textWidget.getTextRange(caret, 1));
	 ...
	 */
	public HydraConstBracketSet(hydraconstraints.resource.hydraConst.ui.HydraConstEditor editor, org.eclipse.jface.text.source.ISourceViewer sourceViewer) {
		languageID = new hydraconstraints.resource.hydraConst.mopp.HydraConstMetaInformation().getSyntaxName();
		this.bracketPairs = new java.util.ArrayList<hydraconstraints.resource.hydraConst.IHydraConstBracketPair>();
	 ...
	 */
	public hydraconstraints.resource.hydraConst.IHydraConstBracketPair getBracketPair(String opening, String closing) {
		for (hydraconstraints.resource.hydraConst.IHydraConstBracketPair bracketPair : bracketPairs) {
	...
	public hydraconstraints.resource.hydraConst.IHydraConstBracketPair getBracketPair(int index) {
		try {
	 ...
	 */
	public boolean setClosingEnabledInside(hydraconstraints.resource.hydraConst.IHydraConstBracketPair bracketPair, boolean closingEnabledInside) {
	...
	public boolean resetBrackets() {
		String bracketPairs = preferenceStore.getString(languageID + hydraconstraints.resource.hydraConst.ui.HydraConstPreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	 ...
	 */
	public hydraconstraints.resource.hydraConst.IHydraConstBracketPair remove(String opening, String closing) {
		for (hydraconstraints.resource.hydraConst.IHydraConstBracketPair bracketPair : bracketPairs) {
			if (bracketPair.getOpeningBracket().equals(opening) && bracketPair.getClosingBracket().equals(closing)) {
	...
	public boolean setBrackets(String bracketSet) {
		...
		bracketPairs = new java.util.ArrayList<hydraconstraints.resource.hydraConst.IHydraConstBracketPair>();
	...
	public void matchingBrackets() {
		...
		if (position != -1 && position != document.getLength()) {
			positionHelper.addPosition(document, hydraconstraints.resource.hydraConst.ui.HydraConstPositionCategory.BRACKET.toString(), position, 1);
			positionHelper.addPosition(document, hydraconstraints.resource.hydraConst.ui.HydraConstPositionCategory.BRACKET.toString(), caretOffset - 1, 1);
		}
	}

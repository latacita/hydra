...
public class HydraConstMarkerResolutionGenerator implements org.eclipse.ui.IMarkerResolutionGenerator {
	...
	public org.eclipse.ui.IMarkerResolution[] getResolutions(org.eclipse.core.resources.IMarker marker) {
		try {
			...
			if (resource instanceof org.eclipse.core.resources.IFile) {
				...
				rs.getLoadOptions().put(hydraconstraints.resource.hydraConst.IHydraConstOptions.DISABLE_CREATING_MARKERS_FOR_PROBLEMS, "true");
				org.eclipse.emf.ecore.resource.Resource emfResource = rs.getResource(uri, true);
				if (emfResource instanceof hydraconstraints.resource.hydraConst.mopp.HydraConstResource) {
					hydraconstraints.resource.hydraConst.mopp.HydraConstResource customResource = (hydraconstraints.resource.hydraConst.mopp.HydraConstResource) emfResource;
					org.eclipse.emf.ecore.util.EcoreUtil.resolveAll(customResource);
					java.util.Collection<hydraconstraints.resource.hydraConst.IHydraConstQuickFix> quickFixes = getQuickFixes(customResource, marker);
					java.util.List<org.eclipse.ui.IMarkerResolution2> resolutions = new java.util.ArrayList<org.eclipse.ui.IMarkerResolution2>();
					for (final hydraconstraints.resource.hydraConst.IHydraConstQuickFix quickFix : quickFixes) {
						resolutions.add(new org.eclipse.ui.IMarkerResolution2() {
							...
							public void run(org.eclipse.core.resources.IMarker marker) {
								...
								try {
									...
									hydraconstraints.resource.hydraConst.ui.HydraConstUIPlugin.logError("Exception while applying quick fix", e);
								}
							}
			...
			hydraconstraints.resource.hydraConst.ui.HydraConstUIPlugin.logError("Exception while computing quick fix resolutions", e);
		}
		return new org.eclipse.ui.IMarkerResolution[] {};
	}
	
	public java.util.Collection<hydraconstraints.resource.hydraConst.IHydraConstQuickFix> getQuickFixes(hydraconstraints.resource.hydraConst.IHydraConstTextResource resource, org.eclipse.core.resources.IMarker marker) {
		...
		try {
			...
			if (quickFixContexts != null) {
				...
				for (String quickFixContext : quickFixContextParts) {
					hydraconstraints.resource.hydraConst.IHydraConstQuickFix quickFix = resource.getQuickFix(quickFixContext);
					if (quickFix != null) {

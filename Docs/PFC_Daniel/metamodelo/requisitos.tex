...
El primer paso para desarrollar nuestro lenguaje era conocer qué aspecto debía tener y qué restricciones debía satisfacer. Es decir, en primer lugar debemos realizar un proceso que podemos denominar de captura de requisitos para poder comprender qué es lo que tiene que hacer exactamente el lenguaje que se pretende crear.
...
%%============================================================================%%
La restricción $GeneralFacilities[WindowMng and HeaterMng]$ sólo se evaluaría en el subárbol cuya raíz es $GeneralFacilities$. Al incluir la operación de contexto estamos evitando posibles confusiones con las características hijas de $RoomFacilities$ o $FloorFacilities$. Con esta restricción estamos forzando que los hogares inteligentes que construyamos tengan un controlador de ventanas de ámbito general y un controlador de temperatura de ámbito general. Si evaluamos esta restricción en la configuración de la Figura~\ref{fig:smarthomeCompleteConf} el resultado será falso, pues tiene controlador de ventanas pero no de temperatura.

En el segundo caso entran en juego los operadores $all$ (para todo) y $any$ (existe). La operación $all$ solo se evaluará a verdadero si la restricción entre corchetes se cumple para todas las instancias de la característica clonable que actúa como contexto. En caso contrario, la operación se evaluará a falso. La operación $any$ se evaluará a verdadero si la restricción entre corchetes se cumple al menos una vez para todas las instancias de la característica clonable que actúa como contexto. Si la restricción no se cumple para ninguna de las selecciones, la operación $any$ se evaluará a falso.

%%============================================================================%%
%% NOTA(Pablo) : Poner un ejemplo y explicarlo                                %%
%%============================================================================%%

La restricción $all Floor [Room >= 2]$ se ejecutará para todas las instancias de la característica $Floor$, y comprobará para cada una de ellas el número de habitaciones. Si todos los pisos tienen al menos 2 habitaciones, su resultado será verdadero. Al ejecutarla en la configuración de la Figura~\ref{fig:smarthomeCompleteConf} esta restricción se evaluará a falso, pues el primer piso solo tiene una habitación. Sin embargo, si ejecutamos la restricción $any Floor [Room >= 2]$ se evaluará a verdadero, pues en este caso sólo es necesario que uno de los pisos tenga al menos dos habitaciones, y la planta baja las tiene.

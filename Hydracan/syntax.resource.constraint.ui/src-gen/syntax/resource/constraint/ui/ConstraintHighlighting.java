 ...
 */
public class ConstraintHighlighting implements org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener {
	
	private final static syntax.resource.constraint.ui.ConstraintPositionHelper positionHelper = new syntax.resource.constraint.ui.ConstraintPositionHelper();
	...
	private syntax.resource.constraint.ui.ConstraintTokenScanner scanner;
	...
	private syntax.resource.constraint.ui.ConstraintEditor editor;
	private org.eclipse.jface.text.source.projection.ProjectionViewer projectionViewer;
	private syntax.resource.constraint.ui.ConstraintOccurrence occurrence;
	private syntax.resource.constraint.ui.ConstraintBracketSet bracketSet;
	 ...
	 */
	private final class UpdateHighlightingListener implements org.eclipse.swt.events.KeyListener, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.MouseListener, syntax.resource.constraint.IConstraintBackgroundParsingListener {
	 ...
	 * @param editor
	 */
	public ConstraintHighlighting(syntax.resource.constraint.IConstraintTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceviewer, syntax.resource.constraint.ui.ConstraintColorManager colorManager, syntax.resource.constraint.ui.ConstraintEditor editor) {
		this.display = org.eclipse.swt.widgets.Display.getCurrent();
		sourceviewer.getSelectionProvider();
		preferenceStore = syntax.resource.constraint.ui.ConstraintUIPlugin.getDefault().getPreferenceStore();
		this.editor = editor;
		textWidget = sourceviewer.getTextWidget();
		projectionViewer = sourceviewer;
		scanner = new syntax.resource.constraint.ui.ConstraintTokenScanner(textResource, colorManager);
		occurrence = new syntax.resource.constraint.ui.ConstraintOccurrence(textResource, sourceviewer, scanner);
		bracketSet = new syntax.resource.constraint.ui.ConstraintBracketSet(editor, sourceviewer);
		this.colorManager = colorManager;
		isHighlightBrackets = preferenceStore.getBoolean(syntax.resource.constraint.ui.ConstraintPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX);
		bracketColor = colorManager.getColor(org.eclipse.jface.preference.PreferenceConverter.getColor(preferenceStore, syntax.resource.constraint.ui.ConstraintPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR));
		black = colorManager.getColor(new org.eclipse.swt.graphics.RGB(0, 0, 0));
		
		addListeners(editor);
	}
	
	private void addListeners(syntax.resource.constraint.ui.ConstraintEditor editor) {
	...
	private void setBracketHighlighting(org.eclipse.jface.text.IDocument document) {
		...
		org.eclipse.jface.text.Position[] positions = positionHelper.getPositions(document, syntax.resource.constraint.ui.ConstraintPositionCategory.BRACKET.toString());
	...
	private void removeHighlighting() {
		...
		removeHighlightingCategory(document, syntax.resource.constraint.ui.ConstraintPositionCategory.BRACKET.toString());
	}
	...
	private void removeHighlightingCategory(org.eclipse.jface.text.IDocument document, String category) {
		...
		if (category.equals(syntax.resource.constraint.ui.ConstraintPositionCategory.BRACKET.toString())) {
		...
		display.syncExec(new Runnable() {
			public void run() {
				...
				if (selectedEObject != null) {
					setSelection(new syntax.resource.constraint.ui.ConstraintEObjectSelection(selectedEObject, false));
				}
			}
		});
	}

...
public class CreateFeatureGroupActionDelegate implements IObjectActionDelegate{

	public final static String ID = "featureModel.popup.CreateFeatureGroupActionID";
	...
	public void run(IAction action) {
		...
		if (selectedElement != null){
			...
			if (feature.getReferences() == null){
				...
				CompoundCommand cc = new CompoundCommand("Create FeatureGroup and Link");

				// Create the new topic for the other end.
				CreateViewRequest topicRequest = CreateViewRequestFactory.getCreateShapeRequest(FeaturemodelElementTypes.FeatureGroup_2009, selectedElement.getDiagramPreferencesHint());
				...
				topicRequest.setLocation(p.translate(-35, 75));
				...
				IElementType relationElementType = FeaturemodelElementTypes.RelationFG_4012;
				...
				EditPart elementPart = (EditPart) viewer.getEditPartRegistry().get(topicViewAdapter.getAdapter(View.class));
				createFeature((FeatureGroupEditPart) elementPart, p);
	...
	private void createFeature(FeatureGroupEditPart featureGroupEditPart, Point p){
		FeatureGroup featureGroup = (FeatureGroup)((View)featureGroupEditPart.getModel()).getElement();

		CompoundCommand cc = new CompoundCommand("Create Feature and Link");

		// Create the new topic for the other end.
		CreateViewRequest topicRequest = CreateViewRequestFactory.getCreateShapeRequest(FeaturemodelElementTypes.Feature_2008, featureGroupEditPart.getDiagramPreferencesHint());

		// A quick hack to get subtopics to layout to the right, from top to bottom
		topicRequest.setLocation(p.translate(-75, 75));

		ProjectEditPart projectEditPart = (ProjectEditPart) featureGroupEditPart.getParent();
		Project project = (Project)((View)projectEditPart.getModel()).getElement();
		Command createTopicCmd = projectEditPart.getCommand(topicRequest);
		IAdaptable topicViewAdapter = (IAdaptable) ((List<?>) topicRequest.getNewObject()).get(0);
		...
		IElementType relationElementType = FeaturemodelElementTypes.RelationFG_4012;
		ICommand createSubTopicsCmd = new DeferredCreateConnectionViewAndElementCommand(new CreateConnectionViewAndElementRequest(relationElementType,
				((IHintedType) relationElementType).getSemanticHint(), featureGroupEditPart.getDiagramPreferencesHint()), new EObjectAdapter((EObject)featureGroupEditPart.getModel()),
				topicViewAdapter, featureGroupEditPart.getViewer());


		cc.add(new ICommandProxy(createSubTopicsCmd));

		featureGroupEditPart.getDiagramEditDomain().getDiagramCommandStack().execute(cc);

		// put the new topic in edit mode
		final EditPartViewer viewer = featureGroupEditPart.getViewer();

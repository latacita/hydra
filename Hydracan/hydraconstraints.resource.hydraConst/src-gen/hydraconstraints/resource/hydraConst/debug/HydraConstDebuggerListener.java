 ...
 * code).
 */
public class HydraConstDebuggerListener<ResultType, ContextType> implements Runnable {
	...
	private hydraconstraints.resource.hydraConst.debug.AbstractHydraConstDebuggable debuggable;
	/**
	 ...
	 */
	private java.util.Map<Long, hydraconstraints.resource.hydraConst.util.HydraConstPair<String, Object>> objectMap = new java.util.LinkedHashMap<Long, hydraconstraints.resource.hydraConst.util.HydraConstPair<String, Object>>();
	...
	public HydraConstDebuggerListener(int requestPort) {
		super();
	...
	private void runDebugger() throws java.io.IOException {
		...
		hydraconstraints.resource.hydraConst.debug.HydraConstDebugMessage command;
		while (!stop) {
			command = communicationHelper.receive(reader);
			if (command == null) {
				break;
			}
			if (command.hasType(hydraconstraints.resource.hydraConst.debug.EHydraConstDebugMessageTypes.EXIT)) {
				debuggable.terminate();
				stop = true;
			} else if (command.hasType(hydraconstraints.resource.hydraConst.debug.EHydraConstDebugMessageTypes.RESUME)) {
				debuggable.resume();
			} else if (command.hasType(hydraconstraints.resource.hydraConst.debug.EHydraConstDebugMessageTypes.STEP_OVER)) {
				debuggable.stepOver();
			} else if (command.hasType(hydraconstraints.resource.hydraConst.debug.EHydraConstDebugMessageTypes.STEP_INTO)) {
				debuggable.stepInto();
			} else if (command.hasType(hydraconstraints.resource.hydraConst.debug.EHydraConstDebugMessageTypes.STEP_RETURN)) {
				debuggable.stepReturn();
			} else if (command.hasType(hydraconstraints.resource.hydraConst.debug.EHydraConstDebugMessageTypes.ADD_LINE_BREAKPOINT)) {
			...
			} else if (command.hasType(hydraconstraints.resource.hydraConst.debug.EHydraConstDebugMessageTypes.REMOVE_LINE_BREAKPOINT)) {
				String location = command.getArgument(0);
				int line = Integer.parseInt(command.getArgument(1));
				debuggable.removeLineBreakpoint(location, line);
			} else if (command.hasType(hydraconstraints.resource.hydraConst.debug.EHydraConstDebugMessageTypes.GET_STACK)) {
				final String[] stack = debuggable.getStack();
				String controlStack = hydraconstraints.resource.hydraConst.util.HydraConstStringUtil.encode('#', stack);
				hydraconstraints.resource.hydraConst.debug.HydraConstDebugMessage message = new hydraconstraints.resource.hydraConst.debug.HydraConstDebugMessage(hydraconstraints.resource.hydraConst.debug.EHydraConstDebugMessageTypes.GET_STACK_RESPONSE, new String[] {controlStack});
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(hydraconstraints.resource.hydraConst.debug.EHydraConstDebugMessageTypes.GET_FRAME_VARIABLES)) {
				...
				hydraconstraints.resource.hydraConst.debug.HydraConstDebugMessage message = new hydraconstraints.resource.hydraConst.debug.HydraConstDebugMessage(hydraconstraints.resource.hydraConst.debug.EHydraConstDebugMessageTypes.GET_FRAME_VARIABLES_RESPONSE, topVariableIDs);
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(hydraconstraints.resource.hydraConst.debug.EHydraConstDebugMessageTypes.GET_VARIABLES)) {
				...
				hydraconstraints.resource.hydraConst.debug.HydraConstDebugMessage message = new hydraconstraints.resource.hydraConst.debug.HydraConstDebugMessage(hydraconstraints.resource.hydraConst.debug.EHydraConstDebugMessageTypes.GET_VARIABLES_RESPONSE, varStrings);
				communicationHelper.sendEvent(message, output);
			} else {
	...
	private String convertToString(long id, Object object) {
		...
		return hydraconstraints.resource.hydraConst.util.HydraConstStringUtil.convertToString(properties);
	}
	...
	private long getObjectID(String name, Object value) {
		hydraconstraints.resource.hydraConst.util.HydraConstPair<String, Object> pair = new hydraconstraints.resource.hydraConst.util.HydraConstPair<String, Object>(name, value);
	...
	public hydraconstraints.resource.hydraConst.debug.AbstractHydraConstDebuggable getDebuggable() {
		return debuggable;
	}
	
	public void setDebuggable(hydraconstraints.resource.hydraConst.debug.AbstractHydraConstDebuggable debuggable) {

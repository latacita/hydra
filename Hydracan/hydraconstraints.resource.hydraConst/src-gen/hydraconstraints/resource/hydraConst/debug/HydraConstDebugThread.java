...
public class HydraConstDebugThread extends hydraconstraints.resource.hydraConst.debug.HydraConstDebugElement implements org.eclipse.debug.core.model.IThread, hydraconstraints.resource.hydraConst.debug.IHydraConstDebugEventListener {
	
	private boolean suspended = false;
	private hydraconstraints.resource.hydraConst.debug.HydraConstDebugTarget debugTarget;
	
	public HydraConstDebugThread(hydraconstraints.resource.hydraConst.debug.HydraConstDebugTarget target) {
		super(target);
	...
	public org.eclipse.debug.core.model.IStackFrame[] getStackFrames() throws org.eclipse.debug.core.DebugException {
		if (isSuspended()) {
			hydraconstraints.resource.hydraConst.debug.HydraConstDebugMessage stack = this.debugTarget.getDebugProxy().getStack();
			String framesData = stack.getArgument(0);
			if (framesData != null && !"".equals(framesData)) {
				java.util.List<String> frames = hydraconstraints.resource.hydraConst.util.HydraConstStringUtil.decode(framesData, '#');
				...
				for (int i = 0; i < frames.size(); i++) {
					...
					theFrames[frames.size() - i - 1] = new hydraconstraints.resource.hydraConst.debug.HydraConstStackFrame(getTarget(), data);
				}
				return theFrames;
			}
		}
	...
	public hydraconstraints.resource.hydraConst.debug.HydraConstDebugTarget getTarget() {
		return debugTarget;
	}

 ...
 */
public class HydraConstDebuggableInterpreter<ResultType, ContextType> extends hydraconstraints.resource.hydraConst.debug.AbstractHydraConstDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private hydraconstraints.resource.hydraConst.util.AbstractHydraConstInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private hydraconstraints.resource.hydraConst.IHydraConstCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public HydraConstDebuggableInterpreter(hydraconstraints.resource.hydraConst.util.AbstractHydraConstInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new hydraconstraints.resource.hydraConst.IHydraConstInterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = hydraconstraints.resource.hydraConst.util.HydraConstStringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public hydraconstraints.resource.hydraConst.util.AbstractHydraConstInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		hydraconstraints.resource.hydraConst.IHydraConstLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private hydraconstraints.resource.hydraConst.IHydraConstLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
		org.eclipse.emf.ecore.resource.Resource resource = element.eResource();
		if (resource instanceof hydraconstraints.resource.hydraConst.IHydraConstTextResource) {
			hydraconstraints.resource.hydraConst.IHydraConstTextResource textResource = (hydraconstraints.resource.hydraConst.IHydraConstTextResource) resource;
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		hydraconstraints.resource.hydraConst.IHydraConstCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(hydraconstraints.resource.hydraConst.debug.EHydraConstDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = hydraconstraints.resource.hydraConst.util.HydraConstEObjectUtil.getDepth(current);
		stopCondition = new hydraconstraints.resource.hydraConst.IHydraConstCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = hydraconstraints.resource.hydraConst.util.HydraConstEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = hydraconstraints.resource.hydraConst.util.HydraConstEObjectUtil.getDepth(current) - 1;
		stopCondition = new hydraconstraints.resource.hydraConst.IHydraConstCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {

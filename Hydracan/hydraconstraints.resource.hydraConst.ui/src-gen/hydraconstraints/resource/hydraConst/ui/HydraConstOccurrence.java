 ...
 */
public class HydraConstOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "hydraconstraints.resource.hydraConst.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "hydraconstraints.resource.hydraConst.ui.occurences.declaration";
	...
	private hydraconstraints.resource.hydraConst.ui.HydraConstTokenScanner tokenScanner;
	...
	private hydraconstraints.resource.hydraConst.IHydraConstTextResource textResource;
	 ...
	 */
	public HydraConstOccurrence(hydraconstraints.resource.hydraConst.IHydraConstTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, hydraconstraints.resource.hydraConst.ui.HydraConstTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new hydraconstraints.resource.hydraConst.mopp.HydraConstMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(hydraconstraints.resource.hydraConst.ui.HydraConstBracketSet bracketSet) {
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, hydraconstraints.resource.hydraConst.ui.HydraConstPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addAnnotation(document, hydraconstraints.resource.hydraConst.ui.HydraConstPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, hydraconstraints.resource.hydraConst.ui.HydraConstPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == hydraconstraints.resource.hydraConst.ui.HydraConstPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(hydraconstraints.resource.hydraConst.ui.HydraConstOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(hydraconstraints.resource.hydraConst.ui.HydraConstOccurrence.DECLARATION_ANNOTATION_ID);
	}

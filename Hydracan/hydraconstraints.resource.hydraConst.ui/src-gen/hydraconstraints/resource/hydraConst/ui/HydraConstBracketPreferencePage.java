 ...
 * </ul>
 */
public class HydraConstBracketPreferencePage extends org.eclipse.jface.preference.PreferencePage implements org.eclipse.ui.IWorkbenchPreferencePage {
	
	private static final String[] ALL_LEFT_BRACKETS = new String[] { "{", "(", "[", "<", "\"", "'", };
	private static final String[] ALL_RIGHT_BRACKETS = new String[] { "}", ")", "]", ">", "\"", "'", };
	
	private String BRACKETS_COLOR = hydraconstraints.resource.hydraConst.ui.HydraConstPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR;
	...
	private String language = new hydraconstraints.resource.hydraConst.mopp.HydraConstMetaInformation().getSyntaxName();
	
	private hydraconstraints.resource.hydraConst.ui.HydraConstBracketSet bracketsTmp;
	
	/**
	 * Creates a preference page for bracket setting.
	 */
	public HydraConstBracketPreferencePage() {
		super();
		
		hydraconstraints.resource.hydraConst.IHydraConstMetaInformation metaInformation = new hydraconstraints.resource.hydraConst.mopp.HydraConstMetaInformation();
	...
	public void init(org.eclipse.ui.IWorkbench workbench) {
		setPreferenceStore(hydraconstraints.resource.hydraConst.ui.HydraConstUIPlugin.getDefault().getPreferenceStore());
		setDescription("Define the coloring of matching brackets.");
		
		bracketsTmp = new hydraconstraints.resource.hydraConst.ui.HydraConstBracketSet(null, null);
		for (String languageID : languageIDs) {
			bracketSetTemp.put(languageID, getPreferenceStore().getString(languageID + hydraconstraints.resource.hydraConst.ui.HydraConstPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		}
	}
	...
	private void handleMatchingBracketsSelection() {
		...
		enableCheckbox.setSelection(getPreferenceStore().getBoolean(		hydraconstraints.resource.hydraConst.ui.HydraConstPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		enableClosingInside.setSelection(false);
		matchingBracketsColorButton.setEnabled(getPreferenceStore().getBoolean(		hydraconstraints.resource.hydraConst.ui.HydraConstPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		...
		initializeLanguage();
		bracketsTmp.setBrackets(getPreferenceStore().getString(language + hydraconstraints.resource.hydraConst.ui.HydraConstPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		...
		bracketsList.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int index : itemIndices) {
					hydraconstraints.resource.hydraConst.IHydraConstBracketPair bracketPair = bracketsTmp.getBracketPair(index);
		...
		enableClosingInside.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int idx : itemIndices) {
					hydraconstraints.resource.hydraConst.IHydraConstBracketPair bracketPair = bracketsTmp.getBracketPair(idx);
	...
	protected void performDefaults() {
		enableCheckbox.setSelection(getPreferenceStore().getDefaultBoolean(hydraconstraints.resource.hydraConst.ui.HydraConstPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		matchingBracketsColorButton.setEnabled(enableCheckbox.getSelection());
		...
		bracketSetTemp.put(language, getPreferenceStore().getDefaultString(language + hydraconstraints.resource.hydraConst.ui.HydraConstPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		bracketsTmp.setBrackets(bracketSetTemp.get(language));
		bracketsList.setItems(bracketsTmp.getBracketArray());
		enableClosingInside.setSelection(false);
	}
	...
	private void updateActiveEditor() {
		...
		getPreferenceStore().setValue(hydraconstraints.resource.hydraConst.ui.HydraConstPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX, enableCheckbox.getSelection());
		getPreferenceStore().setValue(language + hydraconstraints.resource.hydraConst.ui.HydraConstPreferenceConstants.EDITOR_BRACKETS_SUFFIX, bracketSetTemp.get(language));
		...
		if (editor != null && editor instanceof hydraconstraints.resource.hydraConst.ui.HydraConstEditor) {
			((hydraconstraints.resource.hydraConst.ui.HydraConstEditor) editor).invalidateTextRepresentation();
		}
	}

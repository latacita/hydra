...
/**
 * A text editor for 'hydraConst' models.
 */
public class HydraConstEditor extends org.eclipse.ui.editors.text.TextEditor implements org.eclipse.emf.edit.domain.IEditingDomainProvider, org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener, org.eclipse.emf.common.ui.viewer.IViewerProvider, hydraconstraints.resource.hydraConst.IHydraConstResourceProvider, hydraconstraints.resource.hydraConst.ui.IHydraConstBracketHandlerProvider, hydraconstraints.resource.hydraConst.ui.IHydraConstAnnotationModelProvider {
	
	private hydraconstraints.resource.hydraConst.ui.HydraConstHighlighting highlighting;
	private org.eclipse.jface.text.source.projection.ProjectionSupport projectionSupport;
	private hydraconstraints.resource.hydraConst.ui.HydraConstCodeFoldingManager codeFoldingManager;
	private hydraconstraints.resource.hydraConst.ui.HydraConstBackgroundParsingStrategy bgParsingStrategy = new hydraconstraints.resource.hydraConst.ui.HydraConstBackgroundParsingStrategy();
	private java.util.Collection<hydraconstraints.resource.hydraConst.IHydraConstBackgroundParsingListener> bgParsingListeners = new java.util.ArrayList<hydraconstraints.resource.hydraConst.IHydraConstBackgroundParsingListener>();
	private hydraconstraints.resource.hydraConst.ui.HydraConstColorManager colorManager = new hydraconstraints.resource.hydraConst.ui.HydraConstColorManager();
	private hydraconstraints.resource.hydraConst.ui.HydraConstOutlinePage outlinePage;
	...
	private hydraconstraints.resource.hydraConst.ui.HydraConstPropertySheetPage propertySheetPage;
	...
	private hydraconstraints.resource.hydraConst.ui.IHydraConstBracketHandler bracketHandler;
	...
	public HydraConstEditor() {
		super();
		setSourceViewerConfiguration(new hydraconstraints.resource.hydraConst.ui.HydraConstEditorConfiguration(this, this, this, colorManager));
	...
	private final class DocumentListener implements org.eclipse.jface.text.IDocumentListener {
		...
		public void documentChanged(org.eclipse.jface.text.DocumentEvent event) {
			bgParsingStrategy.parse(event, getResource(), HydraConstEditor.this);
		}
	}
	...
	private class ModelResourceChangeListener implements org.eclipse.core.resources.IResourceChangeListener {
		public void resourceChanged(org.eclipse.core.resources.IResourceChangeEvent event) {
			...
			try {
					...
					public boolean visit(org.eclipse.core.resources.IResourceDelta delta) {
						...
						if (deltaKind == org.eclipse.core.resources.IResourceDelta.CHANGED && delta.getFlags() != org.eclipse.core.resources.IResourceDelta.MARKERS) {
							...
							if (changedResource != null) {
								...
								hydraconstraints.resource.hydraConst.IHydraConstTextResource currentResource = getResource();
				...
				hydraconstraints.resource.hydraConst.ui.HydraConstUIPlugin.logError("Unexpected Error: ", exception);
			}
		}
	}
	...
	public void initializeEditor() {
		...
		setEditorContextMenuId("hydraconstraints.resource.hydraConst.EditorContext");
		setRulerContextMenuId("hydraconstraints.resource.hydraConst.EditorRuler");
	}
	...
	public void createPartControl(org.eclipse.swt.widgets.Composite parent) {
		...
		highlighting = new hydraconstraints.resource.hydraConst.ui.HydraConstHighlighting(getResource(), viewer, colorManager, this);
		...
		codeFoldingManager = new hydraconstraints.resource.hydraConst.ui.HydraConstCodeFoldingManager(viewer, this);
	}
	...
	private void initializeResourceObject(org.eclipse.ui.IEditorInput editorInput) {
		...
		hydraconstraints.resource.hydraConst.mopp.HydraConstNature.activate(inputFile.getProject());
		...
		hydraconstraints.resource.hydraConst.IHydraConstTextResource loadedResource = (hydraconstraints.resource.hydraConst.IHydraConstTextResource) resourceSet.getResource(uri, false);
		if (loadedResource == null) {
			try {
				...
				hydraconstraints.resource.hydraConst.IHydraConstTextResource currentResource = this.resource;
				...
				if (demandLoadedResource instanceof hydraconstraints.resource.hydraConst.IHydraConstTextResource) {
					setResource((hydraconstraints.resource.hydraConst.IHydraConstTextResource) demandLoadedResource);
				} else {
					// the resource was not loaded by an EMFText resource, but some other EMF resource
					hydraconstraints.resource.hydraConst.ui.HydraConstUIPlugin.showErrorDialog("No EMFText resource.", "The file '" + uri.lastSegment() + "' of type '" + uri.fileExtension() + "' can not be handled by the HydraConstEditor.");
					// close this editor because it can not present the resource
					close(false);
				}
			} catch (Exception e) {
				hydraconstraints.resource.hydraConst.ui.HydraConstUIPlugin.logError("Exception while loading resource in " + this.getClass().getSimpleName() + ".", e);
			}
		} else {
	...
	private void setResource(hydraconstraints.resource.hydraConst.IHydraConstTextResource resource) {
	...
	private Object getOutlinePage() {
		if (outlinePage == null) {
			outlinePage = new hydraconstraints.resource.hydraConst.ui.HydraConstOutlinePage(this);
	...
	public org.eclipse.ui.views.properties.IPropertySheetPage getPropertySheetPage() {
		if (propertySheetPage == null) {
			propertySheetPage = new hydraconstraints.resource.hydraConst.ui.HydraConstPropertySheetPage();
	...
	public void setCaret(org.eclipse.emf.ecore.EObject element, String text) {
		try {
			...
			org.eclipse.jface.text.source.ISourceViewer viewer = getSourceViewer();
			hydraconstraints.resource.hydraConst.IHydraConstTextResource textResource = (hydraconstraints.resource.hydraConst.IHydraConstTextResource) element.eResource();
			hydraconstraints.resource.hydraConst.IHydraConstLocationMap locationMap = textResource.getLocationMap();
			int destination = locationMap.getCharStart(element);
			int length = locationMap.getCharEnd(element) + 1 - destination;
			
			hydraconstraints.resource.hydraConst.IHydraConstTextScanner lexer = getResource().getMetaInformation().createLexer();
			try {
				lexer.setText(viewer.getDocument().get(destination, length));
				hydraconstraints.resource.hydraConst.IHydraConstTextToken token = lexer.getNextToken();
			...
			hydraconstraints.resource.hydraConst.ui.HydraConstUIPlugin.logError("Exception in setCaret()", e);
		}
	}
	...
	protected org.eclipse.jface.text.source.ISourceViewer createSourceViewer(org.eclipse.swt.widgets.Composite parent, org.eclipse.jface.text.source.IVerticalRuler ruler, int styles) {
		org.eclipse.jface.text.source.ISourceViewer viewer = new org.eclipse.jface.text.source.projection.ProjectionViewer(parent, ruler, getOverviewRuler(), isOverviewRulerVisible(), styles) {
			...
			public void setSelection(org.eclipse.jface.viewers.ISelection selection, boolean reveal) {
				if (!HydraConstEditor.this.setSelection(selection, reveal)) {
	...
	public void addBackgroundParsingListener(hydraconstraints.resource.hydraConst.IHydraConstBackgroundParsingListener listener) {
		bgParsingListeners.add(listener);
	}
	
	public void notifyBackgroundParsingFinished() {
		for (hydraconstraints.resource.hydraConst.IHydraConstBackgroundParsingListener listener : bgParsingListeners) {
	...
	public void setBracketHandler(hydraconstraints.resource.hydraConst.ui.IHydraConstBracketHandler bracketHandler) {

 ...
 * code).
 */
public class ConstraintDebuggerListener<ResultType, ContextType> implements Runnable {
	...
	private syntax.resource.constraint.debug.AbstractConstraintDebuggable debuggable;
	/**
	 ...
	 */
	private java.util.Map<Long, syntax.resource.constraint.util.ConstraintPair<String, Object>> objectMap = new java.util.LinkedHashMap<Long, syntax.resource.constraint.util.ConstraintPair<String, Object>>();
	...
	public ConstraintDebuggerListener(int requestPort) {
		super();
	...
	private void runDebugger() throws java.io.IOException {
		...
		syntax.resource.constraint.debug.ConstraintDebugMessage command;
		while (!stop) {
			command = communicationHelper.receive(reader);
			if (command == null) {
				break;
			}
			if (command.hasType(syntax.resource.constraint.debug.EConstraintDebugMessageTypes.EXIT)) {
				debuggable.terminate();
				stop = true;
			} else if (command.hasType(syntax.resource.constraint.debug.EConstraintDebugMessageTypes.RESUME)) {
				debuggable.resume();
			} else if (command.hasType(syntax.resource.constraint.debug.EConstraintDebugMessageTypes.STEP_OVER)) {
				debuggable.stepOver();
			} else if (command.hasType(syntax.resource.constraint.debug.EConstraintDebugMessageTypes.STEP_INTO)) {
				debuggable.stepInto();
			} else if (command.hasType(syntax.resource.constraint.debug.EConstraintDebugMessageTypes.STEP_RETURN)) {
				debuggable.stepReturn();
			} else if (command.hasType(syntax.resource.constraint.debug.EConstraintDebugMessageTypes.ADD_LINE_BREAKPOINT)) {
			...
			} else if (command.hasType(syntax.resource.constraint.debug.EConstraintDebugMessageTypes.REMOVE_LINE_BREAKPOINT)) {
				String location = command.getArgument(0);
				int line = Integer.parseInt(command.getArgument(1));
				debuggable.removeLineBreakpoint(location, line);
			} else if (command.hasType(syntax.resource.constraint.debug.EConstraintDebugMessageTypes.GET_STACK)) {
				final String[] stack = debuggable.getStack();
				String controlStack = syntax.resource.constraint.util.ConstraintStringUtil.encode('#', stack);
				syntax.resource.constraint.debug.ConstraintDebugMessage message = new syntax.resource.constraint.debug.ConstraintDebugMessage(syntax.resource.constraint.debug.EConstraintDebugMessageTypes.GET_STACK_RESPONSE, new String[] {controlStack});
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(syntax.resource.constraint.debug.EConstraintDebugMessageTypes.GET_FRAME_VARIABLES)) {
				...
				syntax.resource.constraint.debug.ConstraintDebugMessage message = new syntax.resource.constraint.debug.ConstraintDebugMessage(syntax.resource.constraint.debug.EConstraintDebugMessageTypes.GET_FRAME_VARIABLES_RESPONSE, topVariableIDs);
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(syntax.resource.constraint.debug.EConstraintDebugMessageTypes.GET_VARIABLES)) {
				...
				syntax.resource.constraint.debug.ConstraintDebugMessage message = new syntax.resource.constraint.debug.ConstraintDebugMessage(syntax.resource.constraint.debug.EConstraintDebugMessageTypes.GET_VARIABLES_RESPONSE, varStrings);
				communicationHelper.sendEvent(message, output);
			} else {
	...
	private String convertToString(long id, Object object) {
		...
		return syntax.resource.constraint.util.ConstraintStringUtil.convertToString(properties);
	}
	...
	private long getObjectID(String name, Object value) {
		syntax.resource.constraint.util.ConstraintPair<String, Object> pair = new syntax.resource.constraint.util.ConstraintPair<String, Object>(name, value);
	...
	public syntax.resource.constraint.debug.AbstractConstraintDebuggable getDebuggable() {
		return debuggable;
	}
	
	public void setDebuggable(syntax.resource.constraint.debug.AbstractConstraintDebuggable debuggable) {

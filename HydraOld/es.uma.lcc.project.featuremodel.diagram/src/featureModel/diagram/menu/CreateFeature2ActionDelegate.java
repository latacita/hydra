...
public class CreateFeature2ActionDelegate implements IObjectActionDelegate{

	public final static String ID = "featureModel.popup.CreateFeature2ActionID";
	...
	public void run(IAction action) {
		...
		if (selectedElement != null){
			...
			if (feature.getFather() != null){
				EditPart grandpaEditPart = selectedElement.findEditPart(selectedElement.getParent(), feature.getFather());

				CompoundCommand cc = new CompoundCommand("Create Feature and Link");

				// Create the new topic for the other end.
				CreateViewRequest topicRequest = CreateViewRequestFactory.getCreateShapeRequest(FeaturemodelElementTypes.Feature_2008, ((GraphicalEditPart) grandpaEditPart).getDiagramPreferencesHint());

				Point p = ((AbstractGraphicalEditPart) grandpaEditPart).getFigure().getBounds().getTopRight().getCopy();
				((AbstractGraphicalEditPart) grandpaEditPart).getFigure().translateToAbsolute(p);
				int edgeCount = ((GraphicalEditPart) grandpaEditPart).getNotationView().getSourceEdges().size();
				...
				ProjectEditPart projectEditPart = (ProjectEditPart) grandpaEditPart.getParent();
				...
				if (grandpaEditPart instanceof FeatureEditPart ) relationElementType = FeaturemodelElementTypes.RelationFeature_4013;
				ICommand createSubTopicsCmd = new DeferredCreateConnectionViewAndElementCommand(new CreateConnectionViewAndElementRequest(relationElementType,
						((IHintedType) relationElementType).getSemanticHint(), ((GraphicalEditPart) grandpaEditPart).getDiagramPreferencesHint()), new EObjectAdapter((EObject)grandpaEditPart.getModel()),
						topicViewAdapter, grandpaEditPart.getViewer());


				cc.add(new ICommandProxy(createSubTopicsCmd));

				((GraphicalEditPart) grandpaEditPart).getDiagramEditDomain().getDiagramCommandStack().execute(cc);



				// put the new topic in edit mode
				final EditPartViewer viewer = grandpaEditPart.getViewer();
